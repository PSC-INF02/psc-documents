...
Toutefois, le résumé extractif souffre de nombreux défauts liés à l'absence de reformulation des phrases du texte : en définitive, le résumé est limité par la clarté du texte de départ et par l'information qu'il contient explicitement. C'est pourquoi nous voulions mettre l'accent sur une analyse sémantique, afin d'introduire un véritable facteur de \textit{sens}, et reformuler les idées contenues dans le texte, ce qui nous rapproche du résumé abstractif (\textit{abstractive summarization})\index{Abstractive summarization}.\\
...
\paragraph{Workspace}
Le Workspace contient des instances des concepts du Slipnet~: c'est une mémoire de travail.% Il lui arrive aussi de parler de \textit{blackboard} (tableau noir).
...
\`A partir de texte brut, les manipulations suivantes permettent de récupérer les noms propres et les concepts sous forme normale\index{forme normale}~:
\begin{itemize}
 ...
 \item Il reste alors des adjectifs, adverbes, noms et verbes. Nous utilisons \pyt{morphy} de WordNet\index{WordNet} (\ref{Subsection:WordNet}) à l'intérieur d'un code dédié initialement à la construction de Conceptnet5 (\ref{Subsection:Conceptnet5}), pour transformer chaque terme en sa forme normale~;
...
\begin{itemize}
 \item On utilise une requête \verb|LookUp|\index{LookUp} pour avoir une première idée de ces voisins (une liste de liens entrants et sortants auxquels il appartient)~;
...
Le réseau de concepts\index{Réseau de concepts} est la ``mémoire à long terme'' de notre programme. L'objectif est de disposer d'une représentation souple (sous forme de graphe sémantique) de données conceptuelles qui puissent ensuite être utilisées pour mieux comprendre le texte lu.
%% Not really up to date. Advise suppression of last part.
...
%% Once again, great as an upgrade to our program, but we're not doing that
...
% Il évolue au cours de la lecture du texte, notamment par un processus d'activation des n\oe{}uds. L'écoulement du temps est donc une donnée importante.
...
\begin{itemize}
  \item Ou bien introduire un facteur de désactivation des n\oe{}uds au cours du temps~;%% RQ : L'activation s'assimilant à un procédé de chaîne de Markov, la limite est indépendante de l'entrée
...
\end{verbatim}
\end{itemize}
Nous pourrions aller plus loin, mais cela nous menacerait d'arriver à des n\oe{}uds fortement éloignés des deux sportifs initialement sélectionnés, et donc véritablement inutiles.%, et qui s'activeraient entre eux.
...
Le réseau est enregistré sous forme de JSON Stream\index{JSON Stream}, un type de données repris à Conceptnet5, qui permet sans contrainte technique majeure de l'écrire sous forme facilement lisible par un utilisateur extérieur. Il s'agit tout simplement d'une liste d'objets Python pouvant être transformés en JSON\index{JSON} (typiquement, n\oe{}uds et arêtes du graphe), écrits l'un après l'autre, ligne par ligne dans un fichier.\\

Le stade suivant consisterait bien sûr à utiliser une base de données pour stocker et traiter le réseau, mais le nombre de n\oe{}uds étant resté raisonnable, nous n'avons pas eu à abandonner le package \pyt{Networkx}\index{Networkx} qui nous permet de le représenter en mémoire sous forme de graphe.
...
\end{itemize}
% c'est comme ça que je comprends le logarithme, à check
...
Le produit fini du projet, à savoir l'algorithme de résumé, n'est pas le seul outil que nous avons eu à mettre en œuvre dans le cadre du projet. Nous avons en effet amélioré quelques outils de traitement de données textuelles brutes (en ce qui concerne le traitement des noms propres notamment), qui pourraient être réutilisés pour des traitements similaires. Nous avons également pu mettre en place les structures indispensables à nos algorithmes et créé les fonctions faisant l'interface avec les structures de données déjà existantes.

Nous avons, d'autre part, pu implémenter un algorithme raisonnable de résumé automatique s'appuyant sur TF-IDF. Les résultats produits (voir un exemple en annexe B) sont raisonnables, bien qu'un peu flous (il ne s'agissait pas de notre priorité principale).

Pour ce qui est de l'organisation du temps et des ressources, les réunions régulières nous ont permis de rester en synergie et de garder des modules indépendants mais compatibles pendant la majeure partie du développement.
