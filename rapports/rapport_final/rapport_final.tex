...
L'activation d'un mot (provoquée par exemple par sa lecture dans un texte) est propagée à tous les n\oe uds voisins, selon un processus décrit dans la partie [].
...
Chaque nouveau concept donne lieu à une requête de type Lookup. Nous sélectionnons parmi les résultats les informations les plus pertinentes (en terme de poids, mais aussi en fonction des relations, que nous ne gardons pas toutes~: par exemple, RelatedTo n'est pas une relation très pertinente, alors que IsA est souvent pertinente).
...
Nous prenons l'exemple du dernier réseau de concepts que nous avons généré.

\paragraph{Étape 1 : premières requêtes}

Les requêtes Freebase liées aux noms propres créent 10405 n\oe uds dans le réseau, ainsi que 8255 arêtes.

On lance immédiatement des requêtes Search correspondant à tous les n\oe uds du réseau sur Conceptnet5, afin d'acquérir plus d'information : on passe alors à 14103 n\oe uds et 28393 arêtes.

On Lookup sur l'ensemble des concepts trouvés dans les articles (27070), on passe alors à 35780 n\oe uds et 56942 arêtes. Pour le moment, 18762 n\oe uds sont certains d'être conservés (ce sont des résultats primaires de requêtes positives).

On étend alors l'ensemble des n\oe uds encore litigieux (17018), en essayant de créer des liens vers des n\oe uds déjà existants, augmentant ainsi la connectivité du réseau. On passe à 76497 arêtes.

On regarde quels n\oe uds sont bien reliés à leur voisins, après cette opération. Au total, 28583 n\oe uds semblent prometteurs.

On supprime les autres n\oe uds. On obtient un réseau avec 28583 n\oe uds et 69307 arêtes.
...
Le réseau est enregistré sous forme de JSON Stream\index{JSON Stream}, un type de données repris à Conceptnet5. Le nombre de nœuds étant resté raisonnable, nous n'avons pas eu à abandonner prématurément le package \pyt{networkx} qui nous permet de le représenter en mémoire.
