...
La méthode statistique consiste à extraire des phrases par des techniques de la fouille de données textuelles. L'idée principale est de pondérer les phrases selon leur représentativité dans le texte et à afficher celles de poids le plus fort dans l'ordre de leur apparition dans le texte et dans la limite de la taille du résumé. Nous avons choisit ici l'indice TF-IDF pour mesurer ces poids.

\subsection{Algorithme statistique}

Pour donner à chaque phrase un poids, on cherche d'aboard à mesurer l'importance de chaque mot. On a observé que, il y a des mots qui se présentent prêsque dans tous les documents comme les articles, les auxilliaires etc. Donc il faut filtrer ces mots non-significatifs. Et en autre part, les mots-clés d'un texte est aussi des mots qui sont fréquent qui doivent être distribués un poids important.

Donc on cherche ici un indice de mot qui diminue selon le nombre d'apparition dans tous les documents et qui augmente selon le nombre d'apparition dans le texte particulier. Et l'indice TF-IDF satisfait exectement ces charactères.

\subsection{TF-IDF pour le résumé automatique}

\subsubsection{Définition formelle}
\begin{itemize}
\item Fréquence du terme

Selon une lemme de Hans Peter Luhn, le poids d'un terme est propotionnel à son fréquence. La fréquence d'un terme(term frequency) que l'on mesure ici est juste le nombre d'occurences de ce terme dans le document considéré.

\item Fréquence inverse de document

La fréquence inverse de document(inverse document frequency)est une mesure de l'importance du terme dans l'ensemble du corpus. Elle vise à donner un poids plus important aux termes les moins fréquents, considérés comme plus discriminants.

D'après la loi de Zipf, la fréquence d'occurence f(n) d'un mot ext liée à song rang n dans l'ordre des fréquences par une loi de la forme $f(n) = \frac{K}{n} $ où K est une constante.

Donc ici, on prend la logarithme de cette fraction:
...
Plus un mot apparaît, plus son indice idf tend vers à zero. Des mots qui se présent dans tous les documents vont être filtré car ils ont une indice null.

\item TF-IDF

En fin, le poids du mot s'obtient en multipliant les deux mesures.
...
Un évènement est une arête dans le graphe présent dans le workspace, reliant une entité ou un attribut à un autre.
\end{definition}
